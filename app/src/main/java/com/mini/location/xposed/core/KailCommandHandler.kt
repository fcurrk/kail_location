...
import com.mini.location.xposed.hooks.LocationServiceHook
...
internal object KailCommandHandler {
    ...
    fun handle(provider: String?, command: String?, out: Bundle?): Boolean {
        ...
        if (command == "exchange_key") {
            ...
            MiniLog.d(null, "XPOSED", "KAIL接收：交换密钥", isHighFrequency = true)
            return true
        }
        ...
        when (commandId) {
            "is_start" -> {
                ...
                MiniLog.d(null, "XPOSED", "KAIL接收：查询启动状态 is_start=${FakeLocState.isEnabled()}")
                return true
            }
            "start" -> {
                ...
                MiniLog.d(null, "XPOSED", "KAIL接收：启动仿真 altitude=${out.getDouble("altitude", Double.NaN)}")
                return true
            }
            "stop" -> {
                ...
                MiniLog.d(null, "XPOSED", "KAIL接收：停止仿真（已复位所有开关）")
                return true
            }
            "get_location" -> {
                ...
                if (loc != null) {
                    ...
                    MiniLog.d(null, "XPOSED", "KAIL接收：获取位置 lat=${loc.latitude} lon=${loc.longitude}", isHighFrequency = true)
                    return true
                }
                MiniLog.d(null, "XPOSED", "KAIL接收：获取位置失败", isHighFrequency = true)
                return false
            }
            "get_listener_size" -> {
                out.putInt("size", LocationServiceHook.locationListeners.size)
                MiniLog.d(null, "XPOSED", "KAIL接收：监听器数量 size=${LocationServiceHook.locationListeners.size}", isHighFrequency = true)
                return true
            }
            "broadcast_location" -> {
                LocationServiceHook.callOnLocationChanged()
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：广播当前位置", isHighFrequency = true)
                return true
            }
            "set_speed" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：设置速度 speed=$speed", isHighFrequency = true)
                return true
            }
            "set_bearing" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：设置航向 bearing=$bearing", isHighFrequency = true)
                return true
            }
            "set_altitude" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：设置海拔 altitude=$altitude", isHighFrequency = true)
                return true
            }
            "update_location" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：更新位置 lat=$lat lon=$lon", isHighFrequency = true)
                return true
            }
            "set_step_enabled" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：步频开关 enabled=$enabled scheme=$scheme", isHighFrequency = true)
                return true
            }
            "set_step_cadence" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：步频 cadence=$cadence", isHighFrequency = true)
                return true
            }
            "set_step_sim_enabled" -> {
                ...
                out.putBoolean("ok", true)
                MiniLog.d(null, "XPOSED", "KAIL接收：计步器模拟开关 enabled=$enabled", isHighFrequency = true)
                return true
            }
            "load_library" -> {
                ...
                try {
                    ...
                    MiniLog.d(null, "XPOSED", "KAIL接收：加载SO库 path=$path write_offset=$writeOffset result=${result.second}")
                    MiniLog.i(null, "NativeHook", "Library load result: ${result.first}, write_offset=$writeOffset")
                } catch (e: Throwable) {
                    out.putBoolean("ok", false)
                    out.putString("result", e.message ?: "unknown error")
                    MiniLog.e(null, "XPOSED", "KAIL接收：加载SO库失败 path=$path error=${e.message}")
                    MiniLog.e(null, "NativeHook", "Library load error: ${e.message}")
                }
                return true
            }
            "set_route_simulation" -> {
                ...
                try {
                    ...
                    out.putBoolean("ok", true)
                    MiniLog.d(null, "XPOSED", "KAIL接收：路线模拟 active=$active spm=$spm mode=$mode")
                } catch (e: Throwable) {
                    out.putBoolean("ok", false)
                    MiniLog.e(null, "XPOSED", "KAIL接收：设置路线模拟失败 error=${e.message}")
                }
                return true
            }
            "set_config" -> {
                try {
                    ...
                    out.putBoolean("ok", false)
                    MiniLog.e(null, "XPOSED", "KAIL接收：批量配置更新失败 error=${e.message}")
                }
                return true
            }
            else -> return false
