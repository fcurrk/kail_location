package com.mini.location.xposed.hooks.gnss
...
object GnssHook: BaseLocationHook() {
    ...
    private fun hookFrameworkGnss(classLoader: ClassLoader) {
        ...
        val doNothingMethod = object: XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam?) {
                ...
                if (FakeLoc.enableDebugLog) {
                    MiniLog.d(null, "Mini_Xposed", "doNothingMethod: ${param.method.name}")
                }

                if (FakeLoc.enableMockGnss && !FakeLoc.enableAGPS) {
                    if (FakeLoc.enableDebugLog) {
                        MiniLog.d(null, "Mini_Xposed", "${param.method.name}: disable")
                    }
                    param.result = null
                }
            }
        ...
        run {
            ...
            val unhooks = cGnssManagerService.declaredMethods.filter {
                ...
                XposedBridge.hookMethod(method, object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam?) {
                        ...
                        if (FakeLoc.enableDebugLog)
                            MiniLog.d(null, "Mini_Xposed", "registerGnssNmeaCallback: $classListener")
                        kotlin.runCatching {
                            XposedHelpers.findAndHookMethod(classListener, "onNmeaReceived", Long::class.java, String::class.java, object: XC_MethodHook() {
                                override fun beforeHookedMethod(param: MethodHookParam) {
                                    if (FakeLoc.enableMockGnss && !FakeLoc.enableAGPS) {
                                        if (FakeLoc.enableDebugLog)
                                            MiniLog.d(null, "Mini_Xposed", "GnssManagerService.onNmeaReceived: disable")
                                        param.result = null
                                        return
                                    }
                            ...
                            MiniLog.e(null, "Mini_Xposed", "[onNmeaReceived hook failed: ${it.message} from GnssManagerService, please issue?")
                        }
                    }
                })
            }
            ...
            if (FakeLoc.enableDebugLog)
                MiniLog.d(null, "Mini_Xposed", "found ${unhooks.size} registerGnssNmeaCallback")
        }
    }
...
//        hookGnssHALAnyVersion(XposedHelpers.findClassIfExists("android.hardware.gnss.V2_0.IGnss\$Stub", classLoader))
        ...
        if (hookedGnssLocationProvider == 0) {
            MiniLog.e(null, "Mini_Xposed", "Failed to find GnssLocationProvider!")
        } else {
            MiniLog.i(null, "Mini_Xposed", "GnssLocationProvider done, status: $hookedGnssLocationProvider")
        }
        ...
        if (cGnssNative != null) {
            ...
            MiniLog.e(null, "Mini_Xposed", "GnssNative not found")
        }
    }
        ...
        if (cIGnssStub != null) {
            cIGnssStub.hookAllMethods("onTransact", beforeHook {
                ...
                clazz.onceHookAllMethod("injectBestLocation", beforeHook {
                    ...
                    if (FakeLoc.enableDebugLog) {
                        MiniLog.d(null, "Mini_Xposed", "injectLocation: $method")
                    }
                })
            ...
            MiniLog.e(null, "Mini_Xposed", "IGnss\$Stub not found")
        }
    }
}