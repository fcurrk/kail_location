...
object MiniLog {
    ...
    private fun getCallerFileName(): String {
        ...
        // 寻找第一个不是 MiniLog 且不是 Thread 的堆栈帧
    ...
    /**
     * 将日志保存到外部存储的公共目录（Documents/MiniLocation/logs）
    ...
    private fun saveLogToPublicFile(tag: String, message: String, level: Char) {
        logExecutor.execute {
            try {
                ...
                // 仅使用硬编码公共路径
                val publicDir = File("/sdcard/Documents/MiniLocation/logs")
                if (!publicDir.exists()) {
                    publicDir.mkdirs()
                }
                ...
                val fileName = "mini_log_${day}.txt"
                val logEntry = "$ts [${level.uppercaseChar()}] [$tag]: $message\n"
                suAppend("/sdcard/Documents/MiniLocation/logs/$fileName", logEntry)
            }
        }
    }
