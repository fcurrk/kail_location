...
class ServiceGoNoroot : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        MiniLog.i(this, "ServiceGoNoroot", "onCreate started")
        
        try {
            MiniLog.i(this, "ServiceGoNoroot", "1. initNotification")
            initNotification()
        } catch (e: Throwable) {
            MiniLog.e(this, "ServiceGoNoroot", "Error in initNotification: ${e.message}")
        }

        try {
            MiniLog.i(this, "ServiceGoNoroot", "2. init LocationManager")
            mLocManager = this.getSystemService(Context.LOCATION_SERVICE) as LocationManager
        } catch (e: Throwable) {
            MiniLog.e(this, "ServiceGoNoroot", "Error in LocationManager init: ${e.message}")
        }

        try {
            MiniLog.i(this, "ServiceGoNoroot", "3. initGoLocation")
            initGoLocation()
        } catch (e: Throwable) {
            MiniLog.e(this, "ServiceGoNoroot", "Error in initGoLocation: ${e.message}")
        }
            
        try {
            MiniLog.i(this, "ServiceGoNoroot", "4. initJoyStick")
            ...
            MiniLog.e(this, "ServiceGoNoroot", "Error initializing JoyStick: ${e.message}")
            GoUtils.DisplayToast(applicationContext, getString(R.string.service_overlay_failed, e.message))
        }

        broadcastStatus()
        MiniLog.i(this, "ServiceGoNoroot", "onCreate finished")
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
                ...
                when (ctrl) {
                    CONTROL_PAUSE -> {
                        try {
                            ...
                            broadcastStatus()
                            MiniLog.log(this, "ServiceGoNoroot", "Paused simulation (isStop=true)", isHighFrequency = false)
                        } catch (e: Exception) {
                            MiniLog.log(this, "ServiceGoNoroot", "Pause error: ${e.message}", isHighFrequency = false)
                        }
                    ...
                    CONTROL_RESUME -> {
                        try {
                            ...
                            broadcastStatus()
                            MiniLog.log(this, "ServiceGoNoroot", "Resumed simulation (isStop=false)", isHighFrequency = false)
                        } catch (e: Exception) {
                            MiniLog.log(this, "ServiceGoNoroot", "Resume error: ${e.message}", isHighFrequency = false)
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                    CONTROL_STOP -> {
                        try {
                            stopSelf()
                            broadcastStatus()
                            MiniLog.i(this, "ServiceGoNoroot", "stopSelf via control action")
                        } catch (e: Exception) {
                            MiniLog.e(this, "ServiceGoNoroot", "stop error: ${e.message}")
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                    CONTROL_SEEK -> {
                        try {
                            ...
                            if (mRoutePoints.size >= 2 && mRouteCumulativeDistances.isNotEmpty()) {
                                ...
                                MiniLog.i(this, "ServiceGoNoroot", "seek to ratio=$ratio index=$mRouteIndex progress=$mSegmentProgressMeters")
                            }
                        } catch (e: Exception) {
                            MiniLog.e(this, "ServiceGoNoroot", "seek error: ${e.message}")
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                    CONTROL_SET_SPEED -> {
                        try {
                            ...
                            MiniLog.i(this, "ServiceGoNoroot", "speed updated to km/h=$kmh m/s=$mSpeed")
                        } catch (e: Exception) {
                            MiniLog.e(this, "ServiceGoNoroot", "set_speed error: ${e.message}")
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                    CONTROL_SET_SPEED_FLUCTUATION -> {
                        try {
                            ...
                            MiniLog.i(this, "ServiceGoNoroot", "speedFluctuation updated to $speedFluctuation")
                        } catch (e: Exception) {
                            MiniLog.e(this, "ServiceGoNoroot", "set_speed_fluctuation error: ${e.message}")
                        }
                        return super.onStartCommand(intent, flags, startId)
                    }
                }
        ...
        if (mNotification != null) {
            ...
            try {
                ...
                MiniLog.e(this, "ServiceGoNoroot", "Error in onStartCommand initNotification: ${e.message}")
            }
        }
        ...
        if (intent != null) {
            ...
            MiniLog.i(this, "ServiceGoNoroot", "onStartCommand received lat=$mCurLat, lng=$mCurLng")
            ...
            if (this::mJoystickManager.isInitialized) {
                try {
                    ...
                    MiniLog.e(this, "ServiceGoNoroot", "Error setting current position or showing joystick: ${e.message}")
                }
            }
        }
    ...
    override fun onDestroy() {
        MiniLog.i(this, "ServiceGoNoroot", "onDestroy started")
        try {
            val intent = Intent(ACTION_STATUS_CHANGED)
            ...
            MiniLog.e(this, "ServiceGoNoroot", "Error in onDestroy: ${e.message}")
        }

        super.onDestroy()
        MiniLog.i(this, "ServiceGoNoroot", "onDestroy finished")
    }
        ...
        mLocHandler = object : Handler(mLocHandlerThread.looper) {
            override fun handleMessage(msg: Message) {
                try {
                    ...
                    MiniLog.e(this@ServiceGoNoroot, "ServiceGoNoroot", "handleMessage interrupted: ${e.message}")
                    Thread.currentThread().interrupt()
                } catch (e: Exception) {
                    MiniLog.e(this@ServiceGoNoroot, "ServiceGoNoroot", "handleMessage exception: ${e.message}")
                    if (!isStop) {
    ...
    private fun ensureNorootProviders() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "Error ensuring providers: ${e.message}")
        }
    }
    ...
    private fun removeTestProviderGPS() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "removeTestProviderGPS error: ${e.message}")
        }
    }
    ...
    private fun addTestProviderGPS() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "addTestProviderGPS error: ${e.message}")
    ...
    private fun setLocationGPS() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "setLocationGPS error: ${e.message}")
        }
    }
    ...
    private fun removeTestProviderNetwork() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "removeTestProviderNetwork error: ${e.message}")
        }
    }
    ...
    private fun addTestProviderNetwork() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "addTestProviderNetwork error: ${e.message}")
            if (e.message?.contains("not allowed to perform MOCK_LOCATION") == true) {
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(this, getString(R.string.service_set_mock_app), Toast.LENGTH_LONG).show()
                }
    ...
    private fun setLocationNetwork() {
        try {
            ...
            MiniLog.e(this, "ServiceGoNoroot", "setLocationNetwork error: ${e.message}")
        }
    }
